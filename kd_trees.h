...
public:
    ...
    */
    void nearestRec(std::shared<KDNode> node, const Point& pt, size_t depth, double& min_dist, std::shared<KDNode> nearest_node);

    /**
     * @details finds and returns the nearest node to \p pt in the tree
    */
    std::shared<KDNode> nearest(const Point& pt);

    /**
     * @details searaches the tree to find if the point is in the tree recursively
    */
    bool search(std::shared<KDNode> node, const Point& pt, size_t depth);
